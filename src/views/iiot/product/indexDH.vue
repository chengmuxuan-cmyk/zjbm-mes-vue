        ...
        <el-table v-loading="loading" :data="productList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作" width="250" align="center" class-name="small-padding fixed-width">
                <template #default="scope">
                    ...
                    <!--          <el-button link type="primary" icon="Edit" @click="handleUpdateModel(scope.row)" v-hasPermi="['iiot:product:edit']">物模型</el-button>-->
                    <el-button link :type="scope.row.gatewayStatus === 0 ? 'primary' : 'warning'" icon="Edit"
                               @click="handleUpdateGatewayStatus(scope.row)" v-hasPermi="['iiot:product:edit']">
                        {{ scope.row.gatewayStatus === 0 ? "连接" : "断开" }}
                    </el-button>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="productRef" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="设备类型" prop="deviceType">
                    <el-radio-group v-model="form.deviceType" disabled>
                        <el-radio-button
...
import {
    ...
    changeStatusProduct,
    connectDHDevice,
    delProduct, disconnectDHDevice,
    getProduct,
    listProduct,
    updateProduct
} from "@/api/iiot/product";
...
const data = reactive({
    ...
    queryParams: {
        ...
        name: undefined,
        code: undefined,
        productTypeId: '1959168239269883905',
        deviceType: 'Direct',
        status: undefined,
        protocolId: '1959168898580918274',
    },
    rules: {
...
function handleUpdateStatus(row) {
    ...
    proxy.$modal.confirm('是否确认' + (row.status === '1' ? "启用" : "禁用") + '产品名称为"' + productNames + '"的数据项？').then(function () {
...
/** 大华设备连接断开按钮操作 */
function handleUpdateGatewayStatus(row) {
    const productIds = row.id || ids.value;
    const productNames = row.name || names.value;
    proxy.$modal.confirm('是否确认' + (row.gatewayStatus === 0 ? "连接" : "断开") + '产品名称为"' + productNames + '"的数据项？').then(function () {
        proxy.$modal.loading("操作执行中，请稍候...");
        return row.gatewayStatus === 0 ? connectDHDevice(productIds) : disconnectDHDevice(productIds);
    }).then(() => {
        // 等待5秒后再执行后续操作
        setTimeout(() => {
            proxy.$modal.closeLoading();
            getList();
            proxy.$modal.msgSuccess(row.gatewayStatus === 0 ? "连接成功" : "断开成功");
        }, 5000);
    }).catch(() => {
        proxy.$modal.closeLoading();
    });
...
function handleUpdateModel(row) {
    if (row.config?.userName) {
        proxy.$router.push({path: "/iiot/physical-model/index/" + row.id});
    } else {
        proxy.$modal.msgWarning("未配置用户名，无法设置物模型");
    }
}
